...
function checkPwd(strPwd) {
  ...
  var Max = 8;
  if (strPwd == "") {
    alert1("您输入的IC卡/Key密码不能为空");
    isLoginButtonPressed = false;
    return -1;
  }
  for (i = 0; i <= Max; i++) {
    if (strPwd == cPw[i]) {
      alert1("请正确输入8位IC卡/Key密码");
      clearinfo();
      upnode = $("password");
      upnode.value = "";
      upnode.focus();
...
function getValidTimeFromCert(param) {
  EportClient.isInstalledTest(EportClient.cusSpcGetValidTimeFromCert, function (msg, msgJson) {
    ...
    if (overDueDate > 0 && overDueDate <= errorTipDate) {
      alert1("您的IC卡还有" + overDueDate + "天到期，请尽快延期!");
      startLogin(param);
      return;
    }
    if (overDueDate <= 0) {
