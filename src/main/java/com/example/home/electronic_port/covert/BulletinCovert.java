/*
 * Copyright 2006-2021 FacilityONE Inc. All Rights Reserved
 *
 * 注意：
 * 本软件内容仅限于费哲软件内部传阅，禁止外泄以及用于其他商业目的
 * 费哲软件(FacilityONE) : www.facilityone.cn
 */

package com.example.home.electronic_port.covert;

import com.example.home.electronic_port.dto.BulletinDTO;
import com.example.home.electronic_port.entity.BulletinDO;
import org.mapstruct.Mapper;
import org.mapstruct.factory.Mappers;

/**
 * description goes here.
 *
 * @author kid.bian
 * @date 2021/6/8 下午2:14
 * @since 1.0
 **/
@Mapper
public interface BulletinCovert {

    BulletinCovert INSTANCE = Mappers.getMapper(BulletinCovert.class);

    BulletinDO toConvertDO(BulletinDTO bulletinDTO);


}
